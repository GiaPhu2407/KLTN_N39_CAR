    ...
    await Promise.all(
      staffMembers.map((staff) =>
        createNotification({
          ...
          type: "appointment",
          message: `Lịch hẹn mới: ${TenKhachHang} - ${
            lichHenLay.xe?.TenXe || "Xe không xác định"
          } - ${formattedDate} ${formattedTime}`,
        })
      )
    );

    // Always create a notification for the current user who created the appointment
    // Regardless of their role
    await createNotification({
      userId: session.idUsers, // Use the session user ID directly
      type: "appointment",
      message: `Lịch hẹn trải nghiệm xe ${
        lichHenLay.xe?.TenXe || "không xác định"
      } của bạn đã được tạo thành công vào ngày ${formattedDate} lúc ${formattedTime}`,
    });
